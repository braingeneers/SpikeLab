...
**Per-event chunked processing.** When `stim_recording` is a file path or a SpikeInterface recording, the pipeline processes one peri-stim window at a time (loading only `pre_ms + post_ms + ~60 ms` of traces per event) rather than filtering the full recording into RAM. Adjacent events whose windows overlap are merged into a single chunk. This caps peak RAM at ~200 MB on MaxOne regardless of recording duration and makes multi-hour stim recordings tractable. Passing a pre-loaded `ndarray` still runs the full-recording path — use a file path or `MaxwellRecordingExtractor` to get the chunked behavior.
...
**Saturation threshold (`saturation_threshold`)** — when `None` and a recording object is available, a gain-anchored threshold is derived from `recording.get_channel_gains()` combined with the observed amplitude distribution. If no clipping is detected (< 10 samples pinned at the maximum), the threshold returns `+inf` — meaning **no samples get blanked**, and the polynomial detrend handles everything. This matches the "only blank completely saturated electrodes" semantics: high-amplitude artifacts that never hit the ADC rail are recoverable by detrend and should not be destroyed. To force a specific rail, pass the value explicitly (e.g. `saturation_threshold=5500.0`). To fall back to the legacy 99.9-quantile heuristic, call `remove_stim_artifacts` directly without `recording=`.
...
| RT-Sort peak RAM still high (~50–60 GB) on dense MEAs even with all low-RAM options | Detection must hold the full float16 filtered recording + float32 model output + several SpikeInterface filter chains concurrently for a dense MaxOne probe (1018 ch × 20 kHz × 180 s). Lower-RAM chunked detection was considered and rejected (merging sequences across chunks produces unstable results). | Current floor for a 3-minute MaxOne sort is ~50-60 GB peak. Cap the process with `systemd-run --user -p MemoryMax=64G -p ManagedOOMPreference=omit` to prevent the OOM killer from taking other processes. The `ManagedOOMPreference=omit` flag is important — without it, `systemd-oomd`'s 50% pressure policy on the user slice will kill the sort long before it reaches `MemoryMax`. |
