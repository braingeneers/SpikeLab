...
| `ImportError: No module named 'spikeinterface'` (or `natsort` / `six` / `psutil`) on a non-RT-Sort backend | The `[spike-sorting]` extra is not installed in the active environment | Install: `pip install -e ".[spike-sorting]"` (source clone) or `pip install "spikelab[spike-sorting]"` (PyPI). Adds `spikeinterface`, `natsort`, `six`, `pandas`, `psutil`. Kilosort2 also needs MATLAB on PATH or `KILOSORT_PATH` set. For Kilosort4 also install `[kilosort4]` and PyTorch+CUDA: https://pytorch.org/get-started/locally/. Full extras list: `[project.optional-dependencies]` in `pyproject.toml`. |
| `ModuleNotFoundError: No module named 'kilosort'` on the Kilosort4 host backend | The `[kilosort4]` extra is not installed (or the env is missing PyTorch+CUDA) | Install: `pip install -e ".[kilosort4]"` (source clone) or `pip install "spikelab[kilosort4]"` (PyPI), then install PyTorch with CUDA separately: https://pytorch.org/get-started/locally/. Without a local install, run KS4 via Docker: `use_docker=True`. |
| `ImportError: RT-Sort backend dependency check failed — missing: ...` (e.g. `torch`, `spikeinterface`, `diptest`, `tqdm`) | RT-Sort dependencies are missing from the active environment | Install the spike-sorting + ml extras: `pip install -e ".[spike-sorting,ml]"` (source clone) or `pip install "spikelab[spike-sorting,ml]"` (PyPI) — covers `spikeinterface` and `scikit-learn`. Then add the remaining RT-Sort packages: `pip install diptest tqdm`. For `torch`, install a CUDA-matched wheel from https://pytorch.org/get-started/locally/. |
