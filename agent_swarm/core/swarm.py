...
class SwarmOrchestrator:
    ...
    def _init_agents(self):
        ...
        register_swarm_tools(
            ...
            coordinator_tool_defs,
        )

        for agent in [self.researcher, self.engineer, self.validator]:
            register_swarm_tools(
                agent, self.user_proxy, common_tool_map, common_tool_defs
            )
    ...
    def _run_loop(self):
        ...
        print(
            f"DEBUG: Using API key: {key[:5]}...{key[-5:] if len(key) > 10 else ''} (Length: {len(key)})"
        )
    ...
    def _create_pr(self):
        ...
        self.github_tools.create_pr(
            f"Swarm Fix: {self.objective}", "Details in Slack thread."
        )
