...
class TestRunCanaryGlobalsIsolation:
    """``run_canary`` snapshots and restores ``_globals`` so the
    canary's narrower curation / time-slicing overrides do not bleed
    into the next sort. The full sort's already-initialised backend
    only re-syncs globals at ``__init__`` and on OOM scaling, so any
    canary mutation that survives ``run_canary`` is silently consumed
    by the next ``load_recording`` / ``sort`` call.
    """

    def _stub_canary(self, monkeypatch, *, side_effect=None, return_value=None):
        """Wire up enough of the canary internals to drive
        ``run_canary`` to completion without doing real work; the
        stubbed ``process_recording`` mutates ``_globals`` so the
        test can observe whether the restore happened."""
        from spikelab.spike_sorting import (
            backends as backends_mod,
            canary as canary_mod,
            pipeline as pipeline_mod,
        ...
        if side_effect is not None:
            monkeypatch.setattr(pipeline_mod, "process_recording", side_effect)
        else:
            monkeypatch.setattr(
                pipeline_mod, "process_recording", lambda *a, **kw: return_value
            )

    def test_globals_restored_after_success(self, tmp_path, monkeypatch):
        """
        Canary completing normally restores the pre-canary ``_globals``
        state; values mutated by the stubbed ``process_recording``
        do not leak.
        ...
        self._stub_canary(monkeypatch, side_effect=_mutating_process_recording)
    ...
    def test_globals_restored_after_classified_failure(self, tmp_path, monkeypatch):
        """
        Restoration also runs when the canary returns a classified
        failure — the failure path must not leave behind a mutated
        ``_globals`` for the full-sort fallback to consume.

        Tests:
            (Test Case 1) process_recording stub raises a classified
                failure after mutating globals; run_canary returns
                that exception; mutated globals are still restored.
        """
        from spikelab.spike_sorting import _globals
        from spikelab.spike_sorting.canary import run_canary
    ...
    def test_globals_restored_after_keyboard_interrupt(self, tmp_path, monkeypatch):
        """
        ``KeyboardInterrupt`` propagates from inside the canary, but
        the ``finally``-based restore still fires before the
        exception escapes ``run_canary``.
    ...
    def test_snapshot_includes_underscore_chunk_names(self):
        """
        ``_REC_CHUNK_NAMES`` is auto-populated by
        ``concatenate_recordings`` and must be part of the
        snapshot so it does not leak across sorts. The
        snapshot helper accepts this single underscore-prefixed
        name as an exception to its uppercase-prefix filter.

        Tests:
            (Test Case 1) Snapshot dict contains ``_REC_CHUNK_NAMES``;
                restore reverts a mutation.
        """
        from spikelab.spike_sorting import _globals
        from spikelab.spike_sorting.canary import (
            _restore_pipeline_globals,
            _snapshot_pipeline_globals,
        )

        _globals._REC_CHUNK_NAMES = ["a.raw.h5"]
        snap = _snapshot_pipeline_globals()
        assert "_REC_CHUNK_NAMES" in snap
        _globals._REC_CHUNK_NAMES = ["other.raw.h5", "more.raw.h5"]
        _restore_pipeline_globals(snap)
        assert _globals._REC_CHUNK_NAMES == ["a.raw.h5"]
