...
class TestSpikeDataBackendPrintMessages:
    """Tests that parallel processing methods print which backend is used."""

    @staticmethod
    def _make_sd(n_units=4, length=100.0, seed=42):
        """Create a simple SpikeData with enough units for numba thresholds."""
        rng = np.random.default_rng(seed)
        trains = [np.sort(rng.uniform(0, length, size=20)) for _ in range(n_units)]
        return SpikeData(trains, length=length)

    def test_pairwise_sttc_numba_message(self, capsys, monkeypatch):
        """
        pairwise_sttc prints numba backend message when NUMBA_AVAILABLE is True.

        Tests:
            (Test Case 1) With NUMBA_AVAILABLE=True and N>2, prints "numba parallel".
        """
        import spikelab.spikedata.numba_utils as nu

        monkeypatch.setattr(nu, "NUMBA_AVAILABLE", True)
        sd = self._make_sd(n_units=4)
        sd.spike_time_tilings(delt=20.0)
        captured = capsys.readouterr()
        assert "pairwise_sttc: using numba parallel backend" in captured.out

    def test_pairwise_sttc_numpy_message(self, capsys, monkeypatch):
        """
        pairwise_sttc prints numpy backend message when NUMBA_AVAILABLE is False.

        Tests:
            (Test Case 1) With NUMBA_AVAILABLE=False, prints "pure-numpy".
        """
        import spikelab.spikedata.numba_utils as nu

        monkeypatch.setattr(nu, "NUMBA_AVAILABLE", False)
        sd = self._make_sd(n_units=4)
        sd.spike_time_tilings(delt=20.0)
        captured = capsys.readouterr()
        assert "pairwise_sttc: using pure-numpy backend" in captured.out

    def test_pairwise_sttc_numpy_message_small_n(self, capsys):
        """
        pairwise_sttc prints numpy backend message when N <= 2 regardless of numba.

        Tests:
            (Test Case 1) With N=2, always falls back to pure-numpy.
        """
        sd = self._make_sd(n_units=2)
        sd.spike_time_tilings(delt=20.0)
        captured = capsys.readouterr()
        assert "pairwise_sttc: using pure-numpy backend" in captured.out

    def test_pairwise_latencies_numba_message(self, capsys, monkeypatch):
        """
        pairwise_latencies prints numba backend message when conditions are met.

        Tests:
            (Test Case 1) With NUMBA_AVAILABLE=True, N>1, and no distributions,
                prints "numba parallel".
        """
        import spikelab.spikedata.numba_utils as nu

        monkeypatch.setattr(nu, "NUMBA_AVAILABLE", True)
        sd = self._make_sd(n_units=3)
        sd.get_pairwise_latencies()
        captured = capsys.readouterr()
        assert "pairwise_latencies: using numba parallel backend" in captured.out

    def test_pairwise_latencies_numpy_message(self, capsys, monkeypatch):
        """
        pairwise_latencies prints numpy backend message when numba is unavailable.
        ...
        sd.get_pairwise_latencies()
        captured = capsys.readouterr()
        assert "pairwise_latencies: using pure-numpy backend" in captured.out

    def test_pairwise_latencies_numpy_message_distributions(self, capsys):
        """
        pairwise_latencies prints numpy message when return_distributions=True.

        Tests:
            (Test Case 1) return_distributions forces pure-numpy path.
        """
        sd = self._make_sd(n_units=3)
        sd.get_pairwise_latencies(return_distributions=True)
        captured = capsys.readouterr()
        assert "pairwise_latencies: using pure-numpy backend" in captured.out

    def test_spike_triggered_pop_rate_numba_message(self, capsys, monkeypatch):
        """
        compute_spike_trig_pop_rate prints numba message when available.

        Tests:
            (Test Case 1) With NUMBA_AVAILABLE=True, prints "numba parallel".
        """
        import spikelab.spikedata.numba_utils as nu

        monkeypatch.setattr(nu, "NUMBA_AVAILABLE", True)
        sd = self._make_sd(n_units=3, length=200.0)
        sd.compute_spike_trig_pop_rate(window_ms=10, bin_size=1)
        captured = capsys.readouterr()
        assert "spike_triggered_pop_rate: using numba parallel backend" in captured.out

    def test_spike_triggered_pop_rate_numpy_message(self, capsys, monkeypatch):
        """
        compute_spike_trig_pop_rate prints numpy message when numba is unavailable.
        ...
        sd.compute_spike_trig_pop_rate(window_ms=10, bin_size=1)
        captured = capsys.readouterr()
        assert "spike_triggered_pop_rate: using pure-numpy backend" in captured.out

    def test_get_pairwise_ccg_threadpool_message(self, capsys):
        """
        get_pairwise_ccg prints thread pool message with n_jobs=-1.
        ...
        """
        sd = self._make_sd(n_units=3)
        sd.get_pairwise_ccg(max_lag=5, n_jobs=-1)
        captured = capsys.readouterr()
        assert "get_pairwise_ccg: using thread pool" in captured.out
        assert "workers)" in captured.out

    def test_get_pairwise_ccg_serial_message(self, capsys):
        """
        get_pairwise_ccg prints serial message with n_jobs=1.
        ...
        sd.get_pairwise_ccg(max_lag=5, n_jobs=1)
        captured = capsys.readouterr()
        assert "get_pairwise_ccg: using serial backend" in captured.out
