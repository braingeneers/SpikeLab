...
class TestBurstParticipation:
    ...
    def test_compute_burst_participation_auto_save(self):
        ...
        sd.neuron_attributes.compute_burst_participation(
            sd, burst_edges=burst_edges
        )
...
class TestMeanWaveforms:
    ...
    def test_calculate_mean_waveforms_basic(self):
        ...
        for spike_sample in spike_samples:
            if spike_sample < n_samples - 100:
                ...
                raw_data[0, spike_sample-20:spike_sample+20] += np.sin(np.linspace(0, 2*np.pi, 40)) * 5.0
                # Channel 1: smaller spike
                raw_data[1, spike_sample-20:spike_sample+20] += np.sin(np.linspace(0, 2*np.pi, 40)) * 2.0
        
        # Create SpikeData with spike times and raw data
        train = [spike_times_ms]
        sd = SpikeData(train, length=n_samples/fs_khz, raw_data=raw_data, raw_time=fs_khz)
        sd.neuron_attributes = NeuronAttributes.from_dict({'unit_id': [0]}, n_neurons=1)
        ...
        assert wf_data['best_channels'][0] == 0  # Should pick channel 0 (larger amplitude)
    ...
    def test_calculate_mean_waveforms_empty_spike_train(self):
        ...
        # Empty spike train
        train = [np.array([])]
        sd = SpikeData(train, length=n_samples/fs_khz, N=1, raw_data=raw_data, raw_time=fs_khz)
        sd.neuron_attributes = NeuronAttributes.from_dict({'unit_id': [0]}, n_neurons=1)
    ...
    def test_calculate_mean_waveforms_best_channel_selection(self):
        ...
        for spike_sample in spike_samples:
            if spike_sample < n_samples - 50:
                raw_data[0, spike_sample-10:spike_sample+10] += np.sin(np.linspace(0, np.pi, 20)) * 1.0
                raw_data[1, spike_sample-10:spike_sample+10] += np.sin(np.linspace(0, np.pi, 20)) * 2.0
                raw_data[2, spike_sample-10:spike_sample+10] += np.sin(np.linspace(0, np.pi, 20)) * 5.0  # Largest
        
        train = [spike_times_ms]
        sd = SpikeData(train, length=n_samples/fs_khz, raw_data=raw_data, raw_time=fs_khz)
        sd.neuron_attributes = NeuronAttributes.from_dict({'unit_id': [0]}, n_neurons=1)
        
        wf_data = sd.neuron_attributes.calculate_mean_waveforms(sd, ms_before=0.5, ms_after=0.5)
