...
class TestRankOrderCorrelationBackendPrintMessages:
    """Tests that _rank_order_correlation_from_timing prints which backend is used."""

    def test_threadpool_message(self, capsys):
        """
        _rank_order_correlation_from_timing prints thread pool message with n_jobs=-1.
        ...
        rng = np.random.default_rng(42)
        timing = rng.random((5, 6))
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            _rank_order_correlation_from_timing(
                timing, min_overlap=2, n_shuffles=0, n_jobs=-1
            )
        captured = capsys.readouterr()
        assert "rank_order_correlation: using thread pool" in captured.out
        assert "workers)" in captured.out

    def test_serial_message(self, capsys):
        """
        _rank_order_correlation_from_timing prints serial message with n_jobs=1.
        ...
        captured = capsys.readouterr()
        assert "rank_order_correlation: using serial backend" in captured.out
